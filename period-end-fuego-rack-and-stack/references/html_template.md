# HTML Template — Period-End Rack & Stack

This document contains the CSS and HTML structure for the Period-End Fuego
Rack & Stack report. It follows the S3 "Teal Thread" design language.

## Table of Contents

1. [CSS Variables & Base Styles](#css)
2. [Header Component](#header)
3. [KPI Cards](#kpi-cards)
4. [System Summary / GM Message](#gm-message)
5. [System Performance Trends Table](#trends-table)
6. [Rack & Stack Tables](#rack-and-stack)
7. [AI Insight Callouts](#ai-callout)
8. [Print Styles](#print)
...
.report-container { max-width: 100%; margin: 0 auto; padding: 10px;
...
```css
.header { border-radius: 10px; padding: 12px 20px; color: #fff;
  position: relative; overflow: hidden; }
.header-b3 { background: var(--fuego-charcoal); text-align: center;
  padding: 24px 20px 20px; position: relative; }
.header-b3-tag { display: inline-block; font-size: 8px; font-weight: 700;
  letter-spacing: 2px; text-transform: uppercase; padding: 2px 10px;
  border-radius: 3px; margin-bottom: 8px;
  background: rgba(134,202,199,0.15); color: var(--fuego-teal);
  border: 1px solid rgba(134,202,199,0.3); }
.header-b3 h1 { font-size: 28px; font-weight: 800; letter-spacing: -0.5px;
  margin-bottom: 8px; text-shadow: 0 0 40px rgba(134,202,199,0.2); }
.header-b3-meta { display: flex; align-items: center; justify-content: center;
  gap: 8px; font-size: 11px; opacity: 0.75; }
.header-b3-pill { font-size: 9px; font-weight: 600; padding: 2px 8px;
  border-radius: 14px; border: 1px solid rgba(134,202,199,0.5);
  color: var(--fuego-teal); }
...
.b3-v5 .b3-bar-top { height: 2px; background: var(--fuego-teal);
  box-shadow: 0 0 12px rgba(134,202,199,0.6); }
.b3-v5 .b3-bar-bottom { height: 2px; background: var(--fuego-teal);
  box-shadow: 0 0 12px rgba(134,202,199,0.6); }
```
...
.kpi-card .kpi-label { font-size: 9px; font-weight: 500;
...
.kpi-card .kpi-change { display: inline-flex; align-items: center; gap: 3px;
  font-size: 9px; font-weight: 600; margin-top: 6px; padding: 2px 8px;
  border-radius: 4px; }
...
.gm-message .gm-label { font-size: 9px; font-weight: 700;
  text-transform: uppercase; letter-spacing: 1px; color: #5a9e9b;
  margin-bottom: 6px; }
```
...
.section-header .section-sub { font-size: 8px; color: var(--text-secondary);
  margin-left: auto; }
table { width: 100%; border-collapse: collapse; font-size: 10px; }
thead th { padding: 6px 7px; text-align: right; font-weight: 600;
...
tbody td { padding: 6px 7px; text-align: right;
...
```css
.pill { display: inline-block; padding: 1px 5px; border-radius: 4px;
...
.rank { display: inline-block; padding: 1px 6px; border-radius: 4px;
...
```css
.ai-callout { padding: 10px 14px; font-size: 9.5px; line-height: 1.55;
  color: var(--text-primary);
  background: linear-gradient(135deg, #edf7f6, #f4f0ec);
  border-top: 1px dashed var(--fuego-teal); }
.ai-callout .ai-label { font-size: 8px; font-weight: 700;
  text-transform: uppercase; letter-spacing: 1px; color: #5a9e9b;
  margin-bottom: 4px; }
```
...
```css
.footer { text-align: center; font-size: 8px; color: var(--text-secondary);
  padding: 8px 0; }
@media print {
  body { background: var(--bg); -webkit-print-color-adjust: exact;
...
<head>
  <meta charset="UTF-8">
  <title>Fuego System – Period-End Rack & Stack – {REPORT_PERIOD}</title>
  <style>@page{size:A4 portrait;margin:0.3in;}{CSS}</style>
</head>
<body>
<div class="report-container">
  ...
  <div class="header header-b3 b3-v5">
    ...
    <div class="header-b-inner">
      <div class="header-b3-tag">{N}-WEEK CONSOLIDATED RACK &amp; STACK</div>
      <h1>Fuego Tortilla Grill</h1>
      <div class="header-b3-meta">
        <span class="header-b3-pill">All {location_count} Locations</span>
  ...
  <div class="section">
    <div class="section-header">
      ...
      <div class="section-sub">Weekly Breakdown – All Locations Combined</div>
    </div>
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {trends_rows}
        <!-- N-Week Total row with class="total-row" -->
      </tbody>
    </table>
  </div>
  ...
  <div class="section">
    ...
    <table>
      <thead>
        <tr>
          <th>Rank</th><th>Location</th><th>{N}-Wk Sales</th><th>SSS %</th>
  ...
  <div class="section">
    <div class="section-header">
      ...
      <div class="section-sub">{REPORT_PERIOD} – Ranked by Weighted Avg Rating</div>
    </div>
    <table>
      <thead>
        <tr>
  ...
  <div class="section">
    ...
    <table>
      <thead>
        <tr>
          <th>Rank</th><th>Location</th><th>Orders</th>
          <th>Cat $</th><th>Cat $ PY</th><th>vs PY</th>
        </tr>
      </thead>
  ...
  <!-- FOOTER -->
  <div class="footer">
    Generated on {today} · Fuego Tortilla Grill – {N}-Week Consolidated Report · Data sourced from Chabi Analytics
  </div>
